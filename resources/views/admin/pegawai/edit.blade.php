        ...
        <div class="row g-3">
            ...
            <div class="col-12 col-md-4">
                <label class="form-label small fw-semibold text-secondary">ROLE SISTEM <span class="text-danger">*</span></label>
                    ...
                    @foreach($roles as $r)
                        <option value="{{ $r->name }}" {{ (old('role') ?? ($pegawai->roles->first()?->name)) == $r->name ? 'selected' : '' }}>{{ $r->name }}</option>
                    @endforeach
                </select>
                @error('role')
                    <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="col-12 col-md-4">
                ...
                <select name="department_id" class="form-select @error('department_id') is-invalid @enderror">
                    ...
                    @foreach($departments as $Department)
                        <option value="{{ $Department->id }}" {{ old('department_id', $pegawai->department_id) == $Department->id ? 'selected' : '' }}>
                            {{ $Department->name }}
                        </option>
                    @endforeach
                </select>
